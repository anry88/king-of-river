...
- `game/types.ts`: DTOs for profiles, casts, catches, catalog entries, daily reward status, global ratings, snapshots, and persisted profile validation.
...
- `trpc.ts`: shared tRPC router contract and procedure input validation, including the client-timeout `expireCast` path, bait-pack purchase path, daily reward claim path, and rating load/claim paths.
